...
func createIssueHandler(input json.RawMessage) flyte.Event {
	...
	if err != nil {
		...
		log.Println(err)
		return newCreateFailureEvent(err.Error(), handlerInput.Project, handlerInput.IssueType, handlerInput.Summary, handlerInput.Description, handlerInput.Priority)
	}
	return newCreateEvent(fmt.Sprintf("%s/browse/%s", client.JiraConfig.Host, issue.Key), issue.Key, handlerInput.Project, handlerInput.IssueType, handlerInput.Summary, handlerInput.Description, handlerInput.Priority)
}
...
func newCreateFailureEvent(err, project, issueType, summary string, description string, priority string) flyte.Event {
	return flyte.Event{
...
func newCreateEvent(url, id, project, issueType, summary string, description string, priority string) flyte.Event {
	return flyte.Event{
